...
public class PluginLoader {
  ...
  private static void LoadProcess(String className) {
    ...
    if (!(classFile.isAnnotationPresent(EventHandlerComponent.class)
        ...
        || classFile.isAnnotationPresent(MiraiBootComponent.class))) {
      return;
    }
    ...
    for (Method method : classFile.getMethods()) {
      for (Annotation annotation : method.getAnnotations()) {
        if (!(annotation instanceof EventHandler
            || annotation instanceof ExceptionHandler
            || annotation instanceof ConsoleCommand)) {
          continue;
        }
        pluginItem.setAnnotationData(annotation);
